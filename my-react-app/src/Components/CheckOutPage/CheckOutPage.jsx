...
import { clearCart, decreaseItemQuantity, getCartTotal, increaseItemQuantity, removeItem } from "../../Redux/Slice/cartSlice";
        ...
        <div className="cart-container">
            ...
            <div className="cart-left">
                ...
                <div className="items-header">
                    <div>
                        ...
                        <span>{cart.length}/{cart.length} ITEMS SELECTED</span>
                    </div>
                ...
                {/* Item Cards */}
                {cart.map((item, index) => (
                    <div key={`${item.id}-${item.selectedSize}-${index}`} className="item-card">
                        ...
                        <div className="item-details">
                            ...
                            <div className="item-meta">
                                <div>
                                    <span className="label">Size:</span> {item.selectedSize || 'L'}
                                </div>
                                <div className="quantity-button-container">
                                  <button className="quantity-button" onClick={()=>dispatch(decreaseItemQuantity(item.id))}>-</button>
                                    <span className="label">Qty:</span> {item.quantity}
                                    <button className="quantity-button" onClick={()=>dispatch(increaseItemQuantity(item.id))}>+</button>
                                </div>
                            </div>

                            <div className="price-row-11">
                                <div><span className="discounted">₹{item.price * item.quantity}</span></div>
                                <div><span className="original">₹{Math.round(item.price / (1 - item.discount / 100)) * item.quantity}</span></div>
                                <div><span className="off">{item.discount}% OFF</span></div>
                            </div>

                            <p className="return-info">7 days return available</p>
                        </div>
                        <div className="item-remove-container"><span class="close" onClick={()=>dispatch(removeItem(item.id))}>&#10005;</span></div>
                    </div>
                ))}
            </div>
            ...
            <div className="cart-right">
                ...
                <div className="price-details">
                    <h4>PRICE DETAILS ({totalQuantity} {totalQuantity === 1 ? 'Item' : 'Items'})</h4>
                    ...
                    <button className="btn-primary" onClick={() => {navigate('/address')}}>PLACE ORDER</button>
                </div>
            </div>
        </div>
    );
